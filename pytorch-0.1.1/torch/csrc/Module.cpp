...
// 这里通过 import torch,可以获得python的 torch_module
// 将这些映射到 C里面的pyobject
...
// 因为 #define THPTensorStatelessType      TH_CONCAT_2(Real,TensorStatelessType)
// 所以这里PyObject_Call 的就是 PyTypeObject THPTensorStatelessType
// 通过 SetAttrString，将 Stateless变成 TensorClass的一个属性
// STATELESS_ATTRIBUTE_NAME 为 _torch，即 class._torch -> stateless_class
...
// 通过python来callback
// 加载storage、tensor class，将staless挂到 tensorclass上
// 加载对应的转换函数到 转换map里
...
// 当import  torch._C 时触发,
// torch._C Module里包含着有TorchMethods
// module 是pyobject， torch_module 是 C
//  module = PyModule_Create(&torchmodule),这时module 就有了 TorchMethods
// 通过各种 Storage_init(module), tensor_init(module) ,module里就有了各种storageBase、TensorBase
#if PY_MAJOR_VERSION == 2
